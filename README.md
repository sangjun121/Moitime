<p align="center">
  <img src="public/favicon.svg" alt="Timelink logo" width="72" height="72" />
</p>

<h1 align="center">Timelink</h1>

<p align="center">
  모두가 가능한 시간을 빠르게 찾는 일정 조율 서비스
</p>

<p align="center">
  <a href="https://sangjun121.github.io/when-7-meet/"><strong>서비스 바로가기</strong></a>
  ·
  <a href="https://github.com/sangjun121/when-7-meet/issues">버그 제보</a>
  ·
  <a href="https://github.com/sangjun121/when-7-meet/issues">기능 제안</a>
  ·
  <a href="SECURITY.md">Security</a>
</p>

<p align="center">
  <img alt="GitHub Pages" src="https://img.shields.io/badge/GitHub%20Pages-live-19734d?style=flat-square" />
  <img alt="License: MIT" src="https://img.shields.io/badge/license-MIT-1d1d1f?style=flat-square" />
  <img alt="GitHub stars" src="https://img.shields.io/github/stars/sangjun121/when-7-meet?style=flat-square&color=19734d" />
</p>

![Timelink 홈 화면](docs/images/home.png)

## 서비스 소개 영상
<p align="center">
  <a href="https://youtu.be/1qovuK9oxdU">
    <img
      src="docs/images/youtube-thumbnail.png"
      alt="Timelink 서비스 소개 영상"
      width="720"
    />
  </a>
</p>   

**Timelink의 링크 하나로, 서로 가능한 시간을 공유하고, 겹치는 시간을 바로 확인할 수 있습니다.**
- **날짜와 요일**을 기준으로 타임 보드를 만들 수 있습니다.
- 칸을 클릭하거나 드래그해서 가능한 시간을 표시할 수 있습니다.
- 그룹원들이 가능한 시간을 히트맵을 통해 확인할 수 있습니다.
- **구글 캘린더를 연동해 가능한 시간을 자동으로 채울 수 있습니다.**

## 서비스 사진
![Timelink 보드 화면](docs/images/board.png)
![Timelink 예시1 화면](docs/images/example1.png)
![Timelink 예시2 화면](docs/images/example2.png)

## License

Timelink is released under the [MIT License](LICENSE).

## Security

Please read [SECURITY.md](SECURITY.md) before reporting a vulnerability.
